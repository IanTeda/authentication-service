...
use authentication_microservice::rpc::proto::{Empty, SessionsDeleteRequest, SessionsDeleteUserRequest};
...
async fn delete_returns_int(database: Pool<Postgres>) -> Result<()> {
    ...
    let request_message = SessionsDeleteRequest {
...
async fn delete_users_returns_int(database: Pool<Postgres>) -> Result<()> {
    ...
    //-- Execute Test (Act)
    // Build rpc request message
    let request_message = SessionsDeleteUserRequest {
